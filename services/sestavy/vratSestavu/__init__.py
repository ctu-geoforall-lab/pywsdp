...
@brief Base class creating the interface for VratSestavu service
...
class VratSestavu(WSDPBase):
    ...
    def encode_base64(self, string, extension):
        """Decode base64 string"""
        import base64
        binary = base64.b64decode(string)
        output_file = "output.{}".format(extension)
        print(self.get_default_out_dir())
        with open(os.path.join(self.get_default_out_dir(), output_file), 'wb') as f:
            f.write(binary)
